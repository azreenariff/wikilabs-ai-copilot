...
- **Comprehensive UI noise filter** — `is_noise_window_title()` rejects known noise categories: Windows 11/10 Command Palette, Start Menu overlay, MSCTFIME/IME panels, clipboard viewers, toast notifications, generic settings dialogs, and short system dialog titles (properties, security, options, preferences, help, warning, etc.)
- **Terminal REJECTED_CLASSES expanded** — added `Windows.UI.Core.CorePage`, `PopupService`, `#32770` (common dialog), `Dialog`, `Shell_TrayWnd`, `ToastMainWnd`, `FlyoutFrameWindowClass`, `HubWindowClass`, `FlyoutWindow`
- **Terminal content-level UI dialog filter** — unknown-class windows matching UI label patterns ("configuration", "session settings", "preferences", "plugins", "extensions") with ≤5 lines are now rejected
- **`is_terminal_ui_noise()` final filter** — if captured terminal text has UI keywords but NO real terminal content markers (#, $, >, @, root, ping, ssh), it's cleared to empty string
- **Guidance context noise filtering** — `engine.rs` now filters ActiveWindow events before updating the correlation engine, so noise titles never reach the AI guidance system
- **Startup delay** — 5-second buffer before first observation poll to let the desktop settle

- **Terminal/browser window class whitelist/blacklist** — `terminal.rs`, `browser.rs`, and `console_output.rs` now filter child windows by class name instead of relying only on the newline heuristic. Known IME/clipboard/UI chrome classes (`MSCTFIME UI`, `IME`, `msctf_inputPane`, `xwinclip`, `CLIPBRD`, `DirectUIHHost`, `THWindowClass`, `CabinetWClass`, `ToolbarWindow32`, etc.) are explicitly rejected. Known terminal buffer classes (`ConsoleHost_HostAPI`, `CascadiaConsole`, `MobaXtermTerm`, `Chrome_RenderWidgetHostHWND`, `Internet Explorer_Server`, etc.) are accepted regardless of content structure. Unknown classes fall back to the newline heuristic. This eliminates the root cause of the AI hallucinating IME names and clipboard text as "terminal commands" and "page content".
- **Screenshot/vision priority inversion fixed** — the system prompt in `api_server.rs` previously ranked screenshot as #4 (LOW CONFIDENCE) and explicitly told the AI to "IGNORE vision analysis" when terminal commands were visible — but those "terminal commands" were garbage from IME/clipboard windows. Now screenshot/vision is #1 (HIGHEST CONFIDENCE): "The screenshot is the ground truth. Text extraction from Windows UI is a FALLBACK heuristic that can be wrong. If the screenshot shows something, BELIEVE IT."
- **Updated "What to IGNORE" → "When to STAY QUIET"** — replaced the misleading instruction that told AI to distrust screenshots with a clearer "stay quiet" guideline: if the screenshot shows no errors and no meaningful activity, don't give advice.
