...
- **Test Connection no longer fails when API server is initializing** — SetupWizard's `handleTestConnection` and `handleSave` now use exponential backoff retry (up to 3 retries, 1s/2s/4s delays) so the first request can fail without showing "Cannot reach backend" if the API server hasn't finished booting.
...
- **Advice chat window rendering** — Fixed the floating AI chat window showing raw HTML source code instead of the rendered chat interface. The `/advice-chat` route in `api_server.rs` was returning a plain `String` (defaulting to `text/plain`), causing the Tauri webview to display source code. Changed to use Axum's `Html()` wrapper which correctly sets `Content-Type: text/html`.
