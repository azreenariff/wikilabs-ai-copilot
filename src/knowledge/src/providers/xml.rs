...
#[async_trait]
impl KnowledgeProvider for XmlProvider {
    fn name(&self) -> &str {
        "xml"
    }
    ...
    async fn discover(&self, path: &str) -> Result<Vec<ProviderDocument>> {
        ...
        debug!(count = docs.len(), "XML discovery complete");
        Ok(docs)
    }
    ...
    async fn parse(&self, path: &str) -> Result<ProviderDocument> {
        let p = Path::new(path);
        let content =
            fs::read_to_string(p).with_context(|| format!("Failed to read XML file: {}", path))?;
