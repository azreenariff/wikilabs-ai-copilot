...
impl ObservationEngine {
    ...
    ///
    /// This is async (using `lock().await`) rather than sync with `blocking_lock()`
    /// to avoid panics when called from an async task on a single-threaded Tokio runtime.
    /// `blocking_lock()` internally blocks the current thread, which is not allowed
    /// on a single-threaded runtime.
    pub async fn get_last_screenshot(&self) -> Option<crate::screen_capture::CapturedScreenshot> {
        let registry = self.registry.lock().await;
        for provider in registry.all_providers() {
            if provider.provider_type() == ProviderType::ScreenCapture {
                if let Some(screen) = provider.as_any().downcast_ref::<crate::screen_capture::ScreenCaptureProvider>() {
